...
static void applyMovementInput(PlayerState &player, const InputCommand &command,
                               float deltaTime);
static void resolvePlayerCollisions(PlayerState &player,
...
void processPlayerInput(GameState &state, int playerIndex,
                        const InputCommand &command, float deltaTime) {
  if (state.phase != RoundPhase::Active)
    return; // if match is not active, dont allow movement
  ...
  if (!player.sliding && command.crouch && player.grounded &&
    ...
    player.velocity.z *= slideEntryBoost;
             ...
             (!command.crouch || horizontalSpeed < slideMinSpeed)) {
  ...
  // item passive update
  if (player.weapon &&
      (player.weapon == held || player.weapon->isAlwaysActive())) {
    // if we have a weapon, AND we are either holding it or it always ticks,
    // tick it
    player.weapon->passiveUpdate(state, playerIndex, deltaTime);
  }

  if (player.ability &&
      (player.ability == held || player.ability->isAlwaysActive())) {
    // if we have an ability, AND we are either holding it or it always ticks,
    // tick it
  ...
  // dash
  if (command.dash && player.dashCharges > 0 &&
      player.dashTimeLeft <= 0) { //<= 0 checks if we're already dashing
    player.dashCharges -= 1;
...
// like handleInput in Player.cpp but uses commands and server can simulate
// players with it
static void applyMovementInput(PlayerState &player, const InputCommand &command,
                               float deltaTime) {
...
static void resetRound(GameState &state, const std::vector<glm::vec3> spawns) {
  for (int i = 0; i < state.players.size(); i++) {
    ...
    if (i < (int)spawns.size())
      state.players[i].position = spawns[i];
  }

  for (WorldEntity *e : state.worldEntities)
    delete e;
