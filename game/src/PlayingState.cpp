...
void PlayingState::update(float deltaTime) {
  ...
  if (network.hasSnapshot()) {
    ...
    if (snap.tick != lastReconciledTick) {
      ...
      for (int i = 0; i < snap.playerCount && i < (int)gameState.players.size();
           i++) {
        if (i == myIndex)
          continue;
      ...
      for (unsigned int seq = s.lastAppliedSequence + 1;
        ...
        if (c.sequence != seq)
          continue; // slot overwritten, history too short
...
void PlayingState::render() {
  ...
  // the depth buffer since GL_COLOR_BUFFER_BIT and GL_DEPTH_BUFFER_BIT are bit
  // flags, we can combine them with |
  ...
  if (network.hasSnapshot()) {
    ...
    for (int i = 0; i < snap.entityCount; i++) {
      ...
      if (e.type == (int)EntityType::TrafficLight) {
        // if entity is a traffic light

        // first calculate what ActiveState should be:
        glm::vec3 activeState = glm::vec3(0.0f);
        if (e.isRed) {
          activeState = glm::vec3(1.0f, 0.0f, 0.0f);
        } else if (e.isAmber) {
          activeState = glm::vec3(0.0f, 0.0f, 1.0f);
        } else {
          activeState = glm::vec3(0.0f, 1.0f, 0.0f);
        }
  ...
  for (int i = 0; i < gameState.players.size(); i++) {
    ...
    if (network.getInterpolatedPlayer(i, renderTime, interp)) {
      ...
      drawPos = gameState.players[i]
                    .position; // falls back to newest, uninterpreted packet
    }
  ...
  // temporary crosshair:
  uiRenderer.drawRect(w / 2 - 1, h / 2 - 4, 2.0f, 8.0f,
                      glm::vec4(1.0f, 1.0f, 1.0f, 0.4f), w, h); // crosshair y
  uiRenderer.drawRect(w / 2 - 4, h / 2 - 1, 8.0f, 2.0f,
                      glm::vec4(1.0f, 1.0f, 1.0f, 0.4f), w, h); // crosshair x
  ...
  if (healthFraction < 0.0f)
    healthFraction = 0.0f; // clamp to 0
  ...
  // background:
  uiRenderer.drawRect(barX, barY, barWidth, barHeight,
                      glm::vec4(0.1f, 0.1f, 0.1f, 0.6f), w, h);

  // forground
  uiRenderer.drawRect(barX, barY, barWidth * healthFraction, barHeight,
                      glm::vec4(1.0f, 0.2f, 0.2f, 0.8f), w, h);
  ...
  for (int i = 0; i < gameState.roundWins[otherIndex]; i++) {
    ...
    uiRenderer.drawFireRect(fireX, 30 + fireY, fireW, fireH, uiTime, i,
                            ...
                            glm::vec3(0.11373f, 0.23922f, 0.02745f), w, h);

    uiRenderer.drawCircle(px - 3, 30.0 + pipSize + pipGap - 3, pipSize + 6,
                          glm::vec4(0.20000f, 0.01569f, 0.18824f, 1.00000f), w,
                          h); // draw pip border
    uiRenderer.drawCircle(px, 30.0f + pipSize + pipGap, pipSize,
                          glm::vec4(0.92941f, 0.14902f, 0.56471f, 1.00000f), w,
                          h);
  }
  ...
  for (int i = 0; i < gameState.roundWins[myIndex]; i++) {
    ...
    uiRenderer.drawFireRect(fireX, fireY, fireW, fireH, uiTime, i,
                            ...
                            glm::vec3(0.20000f, 0.01569f, 0.18824f), w, h);

    uiRenderer.drawCircle(px - 3, pipY - 3, pipSize + 6,
                          glm::vec4(0.11373f, 0.23922f, 0.02745f, 1.00000f), w,
                          h); // draw pip border
    uiRenderer.drawCircle(px, pipY, pipSize,
                          glm::vec4(0.71765f, 0.92157f, 0.20392f, 1.00000f), w,
